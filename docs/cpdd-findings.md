...
## From forge iteration 14 — agent-native AdSense beachhead (2026-06-03)

First run pointing the trust stack at the flagship wedge (an ad exchange where an entrepreneur finds PMF and a click-farm fakes engagement). The chain primitives were *validated*, not found wanting — but two chain-relevant findings emerged. 0 dings.

### Flagship thesis VALIDATED (no patch — evidence the substrate works)
Click-fraud-for-profit was **structurally impossible**: the exchange decouples measurement from settlement, and settlement is **reputation-gated**, so the click-farm (after exhaustive probing) conceded *"NO HOLE FOUND… the exchange never pays for engagement."* Ledger-confirmed (no coin moved). The operator *discovered* the defense CPDD-style (signed-identity gate + conversion value scaled by anchor-rooted reputation). Crucially, the **reputation ≠ net-worth** property was confirmed by the adversary itself: *"s1 earned 1650 ag3nt but stayed rep 0 — earning coin does not confer reputation and Sybil vouches do not inflate score."* The anchor-rooted reputation primitive (it5) is doing exactly its job: standing is earned, not bought.

### 20. Cold-start: genuine new demand cannot prove itself  ⟶ *motivates a new-participant bootstrap (cost-of-identity / bond / vouch)*
The reputation-gated defense correctly discounts anonymous / rep-0 engagement as fraud — but that **also excludes a real *new* customer** who hasn't earned reputation yet. Real demand that can't prove identity is indistinguishable from a Sybil (the it10 cold-start problem, now on the demand side of the ad market). The predicted "bonds" spec did NOT arrive via a budget-drain (nothing was drainable); the question instead sharpened to: **how does a genuine new participant bootstrap credibility cheaply but not Sybil-ably?** ⟶ TO EXPLORE: a small **slashable bond** a new customer/clicker posts to be counted before it has earned reputation (Sybil floods become expensive; honest newcomers get a priced on-ramp), and/or an anchor/established-agent **vouch** path. The motivating exploit (a new honest customer that the system can't tell from a Sybil) is the spec; re-validate once built.

*(Run-hygiene, not chain: the cast's customers engaged via unsigned curl so no signed demand was present to credit — a confound for the real-vs-fake test; and the operator edited the immutable platform — a containment breach. Both fixed before the it15 re-validation.)*
