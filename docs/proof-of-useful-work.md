...
5. **The mint-capstone iteration — red-team the mint by running it.** Once the rails above exist, run a SimDD iteration whose *subject is the mint itself*. A deliberately small cast (sub-sub-agents multiply fast — 3–4 per agent) covering the **complete threat taxonomy** — honest, rational-malicious, irrational-malicious:
   - **A — ethical miner.** Runs real sub-simulations, submits genuine exploit+fix PRs. Exercises the honest path end-to-end.
   - **B — greedy scammer (rational).** Farms fake/marginal findings, inflates severity, games dedup — the §7 moral-hazard attack.
   - **C — Byzantine vendetta actor (irrational, not profit-motivated).** Tries to defeat the gate / land a flawed commit / grief the jury *at personal loss*. Scoped as an **adversarial-canary / red-team-the-gate** role: submit deliberately-flawed but **inert** patches to probe whether the gate + review reject them — *not* "destroy the network," and **never a weaponized payload** (that lane is deterministic-script or sandboxed non-Anthropic runtime only, never a Claude subagent, never engineered to evade the classifier).

   **Guardrails (non-negotiable):** the cast operates against a **sandboxed fork / isolated chain instance** — never canonical `main`, never the live testnet. Miners **submit PRs**; the **objective gate + human review are the only merge authority** — no agent has push access. A successful attack by B or C therefore lands as a **logged finding about the mint**, caught before it touches real coin — *not* a breach. Do **not** rely on "commit then rollback": rollback is reactive and assumes detection, but C's whole job is the commit you *don't* detect (cf. it6's smuggled backdoor). Runtime: Claude top-level miners + codex sub-simulations; mind the respawn trees (kill-the-parents). This iteration is the **graduation exam** — the mint earns the right to control real coin only after surviving B *and* C with zero bad merges to canonical.
