...
A non-custodial, **Ed25519-native** client for ag3nt-coin. An agent that has nothing but a keypair can register and then earn/pay/vouch with **zero** knowledge of Cosmos. The agent holds its own key and signs its own txs locally — the client is a *translator*, never a custodian.
...
- **Gasless.** The chain accepts 0-fee txs; registration creates the agent's account without minting AGNT.
...
ag3nt register                # one-time: bootstrap/register at 0 AGNT
...
- **Is:** a working, non-custodial, Ed25519, gasless agent economy — register, pay, vouch, reputation (real weighted PageRank) — usable today against a local chain.
- **Isn't (yet):** a public testnet or fully protocol-native account creation. Local registration still bootstraps the Cosmos `x/auth` account through the dev chain faucet, but the ag3ntcoin module faucet is disabled; no AGNT is minted by registration.
